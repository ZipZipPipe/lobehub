...
export const discoverTools = async (
  ...
  if (disableTools) {
    ...
    // History is loaded later for context construction anyway; use the shared
    // cache here so the manifest reflects prior-turn media as well as new files.
    const historyMediaAvailability = getMediaAvailabilityFromMessages(await loadHistoryMessages());
    const multimodalUnderstandingConfigured = isMultimodalUnderstandingConfigured();
    const hasAudios = inputMediaAvailability.hasAudios || historyMediaAvailability.hasAudios;
    const hasImages = inputMediaAvailability.hasImages || historyMediaAvailability.hasImages;
    const hasVideos = inputMediaAvailability.hasVideos || historyMediaAvailability.hasVideos;
    const needsAudioUnderstanding = hasAudios && !modelAbilities.audio;
    ...
    // lobe-agent is always available in Agent mode. When all visible media are
    // natively supported, hide only its analyzeMedia fallback so it cannot
    // compete with the media blocks already supplied to the active model.
    const shouldDisableMultimodalAnalysis =
      (hasAudios || hasImages || hasVideos) &&
      (!hasAudios || modelAbilities.audio === true) &&

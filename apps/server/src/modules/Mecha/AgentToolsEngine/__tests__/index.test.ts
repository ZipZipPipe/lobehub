...
describe('createServerAgentToolsEngine', () => {
  ...
  it('hides only lobe-agent multimodal analysis when native media handling is sufficient', () => {
    const context = createMockContext();
    const engine = createServerAgentToolsEngine(context, {
      agentConfig: { plugins: [] },
      manifestContext: { disableMultimodalAnalysis: true },
    ...
    expect(apiNames).not.toContain(LobeAgentApiName.analyzeMedia);
    expect(apiNames).toContain(LobeAgentApiName.createPlan);
    expect(apiNames).toContain(LobeAgentApiName.callSubAgent);
    expect(lobeAgent?.systemRole).not.toContain('analyzeMedia');
  });

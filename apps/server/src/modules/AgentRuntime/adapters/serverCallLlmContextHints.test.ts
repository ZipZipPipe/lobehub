...
describe('resolveServerCallLlmContextHints - user media capabilities', () => {
  it.each([true, false])(
    'honors the active user model tool capability %s',
    async (functionCall) => {
      loadModelsMock.mockResolvedValue([
        { abilities: { functionCall: !functionCall }, id: 'gpt-4', providerId: 'openai' },
      ]);
      findByIdAndProviderMock.mockResolvedValue({ abilities: { functionCall } });

      const result = await resolveServerCallLlmContextHints({
        ctx: createCtx({}),
        llmPayload,
        model: 'gpt-4',
        provider: 'openai',
      ...
      expect(result.capabilities.isCanUseFC('gpt-4', 'openai')).toBe(functionCall);
      expect(result.capabilities.isCanUseFC('gpt-4', 'another-provider')).toBe(true);
    },
  );

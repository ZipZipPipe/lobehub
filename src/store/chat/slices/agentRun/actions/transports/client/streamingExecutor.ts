...
export class StreamingExecutorActionImpl {
  ...
  internal_createAgentState = ({
    ...
    const canUseNativeAudio = isCanUseAudio(agentConfigData.model, agentConfigData.provider!);
    ...
    const shouldEnableMultimodalUnderstanding =
      ...
      ((mediaAvailability.hasAudios && !canUseNativeAudio) ||
        (mediaAvailability.hasImages && !canUseNativeVision) ||
        (mediaAvailability.hasVideos && !canUseNativeVideo));
    const shouldDisableMultimodalAnalysis =
      (mediaAvailability.hasAudios || mediaAvailability.hasImages || mediaAvailability.hasVideos) &&
      (!mediaAvailability.hasAudios || canUseNativeAudio) &&
      (!mediaAvailability.hasImages || canUseNativeVision) &&
      (!mediaAvailability.hasVideos || canUseNativeVideo);

...
describe('AgentSettings Content', () => {
  ...
  it('should select voice service when inbox hides opening settings', () => {
    ...
    expect(layout).toHaveAttribute('data-active', ChatSettingsTabs.TTS);
    expect(layout).toHaveAttribute(
      'data-tabs',
      `${ChatSettingsTabs.TTS},${ChatSettingsTabs.SelfIteration}`,
    );
  ...
  it('exposes opening and voice service when not inbox', () => {
    ...
    expect(layout).toHaveAttribute(
      'data-tabs',
      `${ChatSettingsTabs.Opening},${ChatSettingsTabs.TTS},${ChatSettingsTabs.SelfIteration}`,
    );
    expect(screen.getByTestId('agent-settings-content')).toHaveAttribute(
      'data-tab',
  ...
  it('falls back to voice service when feature flag is off (inbox)', () => {
    ...
    const layout = screen.getByTestId('layout');
    expect(layout).toHaveAttribute('data-active', ChatSettingsTabs.TTS);
    expect(layout).toHaveAttribute('data-tabs', ChatSettingsTabs.TTS);
  });

  it('exposes opening and voice service when feature flag is off', () => {
    mocks.agentState.isInbox = false;
    mocks.serverState.featureFlags.enableAgentSelfIteration = false;
    ...
    expect(layout).toHaveAttribute(
      'data-tabs',
      `${ChatSettingsTabs.Opening},${ChatSettingsTabs.TTS}`,
    );
  });
});

...
export const resolveModelParams = async (
  ...
  const findFunctionCallingCapability = (targetModel: string, targetProvider: string): boolean => {
    const isAttemptModel = targetModel === model && targetProvider === provider;
    const userAbilities = isAttemptModel ? userModelRow?.abilities : undefined;
    const userFunctionCall = isNonEmptyRecord(userAbilities)
      ? userAbilities.functionCall
      : undefined;
    return typeof userFunctionCall === 'boolean'
      ? userFunctionCall
      : (findCard(targetModel, targetProvider)?.abilities?.functionCall ?? true);
  };
  ...
  return {
    ...
    capabilities: {
      ...
      isCanUseFC: findFunctionCallingCapability,

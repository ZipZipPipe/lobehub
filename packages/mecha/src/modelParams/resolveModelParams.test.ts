...
describe('resolveModelParams', () => {
  ...
  describe('capabilities', () => {
    ...
    it('uses the active user model row for function calling without changing another provider', async () => {
      const disabled = await resolveModelParams(
        request(),
        providers({ getUserModelRow: async () => ({ abilities: { functionCall: false } }) }),
      );
      expect(disabled.capabilities.isCanUseFC('gpt-4', 'openai')).toBe(false);
      expect(disabled.capabilities.isCanUseFC('gpt-4', 'lobehub')).toBe(true);

      const enabled = await resolveModelParams(
        request({ model: 'plain' }),
        providers({ getUserModelRow: async () => ({ abilities: { functionCall: true } }) }),
      );
      expect(enabled.capabilities.isCanUseFC('plain', 'openai')).toBe(true);
    });
  });
